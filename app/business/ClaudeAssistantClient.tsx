...
export default function ClaudeAssistantClient() {
  ...
  async function generate() {
    ...
    try {
      ...
      setResult(data.text || "");
      setStatus(data.usedAi ? "Claude improved the built-in draft." : data.warning || "Built-in result used.");
    } catch (error) { setStatus(error instanceof Error ? error.message : "Generation failed"); }
    finally { setRunning(false); }
  }
  return <div className="ops-two-column">
    <section className="panel stack"><p className="badge">OPTIONAL ASSISTANT</p><h2>Writing assistant</h2><p className="muted">Core business tools do not depend on Claude. When the API is missing, slow or unavailable, this tool returns a built-in draft instead of crashing.</p>
      ...
      <button onClick={generate} disabled={running}>{running ? "Working…" : "Create draft"}</button><p className="muted small">{status}</p>
    </section>
